...
class ProductionLineTester {
	...
	ProductLineTestReport test(Stream<Product> products) {
		...
		if (null != products) {
			...
			return new ProductLineTestReport(count.getCorrectCnt(), count.getCheckedExcCnt(),
					count.getUncheckedExcCnt(),
	...
	class ProductLineTestReport {
		...
		public ProductLineTestReport(long correctProductsInLine, long checkedExceptionsCount,
				long uncheckedExceptionCount, long otherExceptionCount) {
			super();
